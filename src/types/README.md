...
Everything on a map, from the tiles, to the types of terrains on those tiles, the unit types, the commanders, etc. are all "objects." This folder defines the different fields that each category of object should store in the _back end_. For the front end variants of these classes, check out the `public/types` folder.
